package com.nlpeng.es.service.impl;

import com.nlpeng.es.dao.EsProductDao;
import com.nlpeng.es.domain.EsProduct;
import com.nlpeng.es.domain.EsProductRelatedInfo;
import com.nlpeng.es.repository.EsProductRepostictory;
...
public class EsProductServiceImpl implements EsProductService {
	...
	@Autowired
	private EsProductRepostictory productRepostictory;
	...
	public int importAll() {
		...
		Iterable<EsProduct> esProductIterable = productRepostictory.saveAll(esProductList);
	...
	public void delete(Long id) {
		productRepostictory.deleteById(id);
	}
	...
	public EsProduct create(Long id) {
		...
		if (esProductList.size() > 0) {
			...
			result = productRepostictory.save(esProduct);
		}
		return result;
	}
	...
	public void delete(List<Long> ids) {
		if (!CollectionUtils.isEmpty(ids)) {
			...
			productRepostictory.deleteAll(esProductList);
		}
	}
	...
	public Page<EsProduct> search(String keyword, Integer pageNum, Integer pageSize) {
		...
		return productRepostictory.findByNameOrSubTitleOrKeywords(keyword, keyword, keyword, pageable);
	}
	...
	public Page<EsProduct> search(String keyword, Long brandId, Long productCategoryId, Integer pageNum, Integer pageSize, Integer sort) {
		...
		return productRepostictory.search(searchQuery);
	}
	/**
	 * 根据商品id推荐相关商品
	 */
	@Override
	public Page<EsProduct> recommed(Long id, Integer pageNum, Integer pageSize) {
		Pageable pageable = PageRequest.of(pageNum, pageSize);
		List<EsProduct> esProductList = productDao.getAllEsProductList(id);
		if (esProductList.size() > 0) {
			EsProduct esProduct = esProductList.get(0);
			...
			//根据商品标题、品牌、分类进行搜索
			List<FunctionScoreQueryBuilder.FilterFunctionBuilder> filterFunctionBuilders = new ArrayList<>();//功能评分查询生成器
			filterFunctionBuilders.add(new FunctionScoreQueryBuilder.FilterFunctionBuilder(QueryBuilders.matchQuery("name", keyword),
		...
		AbstractAggregationBuilder aggregationBuilder =
				...
				.subAggregation(
						...
						.subAggregation(
								...
								.subAggregation(AggregationBuilders.terms("attrValues").field("atrValueList.value"))
